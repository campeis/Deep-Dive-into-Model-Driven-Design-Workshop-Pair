...
public class RowTest {
    ...
    public void Offer_adjacent_seats_nearer_the_middle_of_the_row_when_the_middle_is_not_reserved() {
        ...
        var seatingPlaces = row.suggestSeatingOption(partySize, PricingCategory.MIXED).seats();
    
        assertThat(seatingPlaces).containsExactly(a5, a6, a7);
    }
    
    @Test
    public void Offer_adjacent_seats_nearer_the_middle_of_the_row_when_the_middle_is_not_reserved_cornerCase_01 () {
        int partySize = 3;
        ...
        SeatingPlace a5  = new SeatingPlace("A",  5, PricingCategory.FIRST,  SeatingPlaceAvailability.RESERVED);
        SeatingPlace a6  = new SeatingPlace("A",  6, PricingCategory.FIRST,  SeatingPlaceAvailability.AVAILABLE);
        SeatingPlace a7  = new SeatingPlace("A",  7, PricingCategory.FIRST,  SeatingPlaceAvailability.RESERVED);
        SeatingPlace a8  = new SeatingPlace("A",  8, PricingCategory.FIRST,  SeatingPlaceAvailability.RESERVED);
        SeatingPlace a9  = new SeatingPlace("A",  9, PricingCategory.SECOND, SeatingPlaceAvailability.AVAILABLE);
        SeatingPlace a10 = new SeatingPlace("A", 10, PricingCategory.SECOND, SeatingPlaceAvailability.AVAILABLE);
    
        Row row = new Row("A",  new ArrayList<>(Arrays.asList(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10 )));
        assertThat(row.adjacentSeats(partySize).size()).isEqualTo(8);

        assertThat(row.bestSeats(partySize, PricingCategory.FIRST).size()).isEqualTo(0);
        assertThat(row.bestSeats(partySize, PricingCategory.MIXED).size()).isEqualTo(2);
        assertThat(row.bestSeats(partySize, PricingCategory.MIXED).get(0).seats()).containsExactly(a2, a3, a4);
